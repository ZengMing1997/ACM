    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < m; j++)
        {
            for(int k = 0 ; k < 4 ; k++)
            {
                ...
                if(G[i][j][k]) Edg_info.emplace_back(Edge(ID(i,j),ID(X,Y)));
            }
            Point_info.emplace_back(Point(i,j,ID(i,j)));
        }
    }
}
void GetMapdetail()
...
void GetLearnedQRError()
{
	...
	{
		printf("{\"robot_index\": \"%d\",\"learned_times\": %d,\"head\": \"%d\",\"tail\": \"%d\"}",index++,rand()%4,i.first,i.second);
